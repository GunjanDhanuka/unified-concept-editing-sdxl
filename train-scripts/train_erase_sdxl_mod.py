...
def text2image_ldm_stable(
    ...
    text_embeddings = ldm_stable.text_encoder(text_input.input_ids.to(ldm_stable.device), output_hidden_states=True)
    text_embeddings = text_embeddings.hidden_states[-2]
    text_input_2 = ldm_stable.tokenizer_2(
        texts,
        padding="max_length",
    ...
    text_embeddings_2 = ldm_stable.text_encoder_2(text_input_2.input_ids.to(ldm_stable.device), output_hidden_states=True)
    text_embeddings_2 = text_embeddings_2.hidden_states[-2]
    # print(text_embeddings.shape, text_embeddings_2.shape)
    ...
    uncond_embeddings = model.text_encoder(uncond_input.input_ids.to(model.device), output_hidden_states=True)
    ...
    uncond_embeddings = torch.concat([uncond_embeddings, uncond_embeddings_2], dim=-1)
...
def edit_model(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale = 0.1, preserve_scale = 0.1, with_to_k=True, technique='tensor'):
    ...
    print("cross attention layers done")
    ...
    ######################## START ERASING ###################################
    print("start erasing")
    for layer_num in tqdm(range(len(projection_matrices)), desc="Processing layers"):
        ...
        with torch.no_grad():
            ...
            for cnt, t in enumerate(zip(old_texts, new_texts)):
                ...
                text_embeddings = ldm_stable.text_encoder(text_input.input_ids.to(ldm_stable.device), output_hidden_states=True)
                text_embeddings = text_embeddings.hidden_states[-2] # 768 dim
                text_input_2 = ldm_stable.tokenizer_2(
                    texts,
                    padding="max_length",
                    max_length=ldm_stable.tokenizer_2.model_max_length,
                ...
                text_embeddings_2 = ldm_stable.text_encoder_2(text_input_2.input_ids.to(ldm_stable.device), output_hidden_states=True)
                text_embeddings_2 = text_embeddings_2.hidden_states[-2] # 1280 dim
                # print(text_embeddings.shape, text_embeddings_2.shape)
                text_embeddings = torch.concat([text_embeddings, text_embeddings_2], dim=-1) # 2048 dim 
                # print(text_embeddings.shape)
                ...
                # print(old_emb.shape, new_emb.shape)
            ...
            #     old_emb, new_emb = text_embeddings[0], text_embeddings[1]
            #     context = old_emb.detach()
            #     values = []
            #     with torch.no_grad():
            #         for layer in projection_matrices:   
            ...
            #     for_mat1 = (value_vector @ context_vector_T).sum(dim=0)
            #     for_mat2 = (context_vector @ context_vector_T).sum(dim=0)
            #     # print(f"mat2 stats before updates : min={mat2.min()}, max={mat2.max()}, mean={mat2.mean()}")
            #     # print(f"Contains nan: {torch.isnan(mat2).any()}")
            #     # print(f"Contains inf: {torch.isinf(mat2).any()}")
            #     mat1 += preserve_scale*for_mat1
            #     interm = preserve_scale*for_mat2
            #     print("for_mat1 stats: min={0}, max={1}, mean={2}".format(for_mat1.min(), for_mat1.max(), for_mat1.mean()))
            #     print("for_mat2 stats: min={0}, max={1}, mean={2}".format(for_mat2.min(), for_mat2.max(), for_mat2.mean()))
            #     print("interm stats: min={0}, max={1}, mean={2}".format(interm.min(), interm.max(), interm.mean()))
            #     # mat2 += interm
            #     print("inner loop done")
            # # import pdb; pdb.set_trace()
            #     #update projection matrix
            # # print(mat1.shape, mat2.shape, projection_matrices[layer_num].weight.shape)
            # mat1 = mat1.to(device)
            # mat2 = mat2.to(device)
            mat1 = mat1.float()
            mat2 = mat2.float()
            projection_matrices[layer_num].weight = torch.nn.Parameter((mat1 @ torch.inverse(mat2)).half())
...
if __name__ == '__main__':
    ...
    # ldm_stable.text_encoder2 = None
    print_text += f"-sd_{args.base.replace('.','_')}" 
    print_text += f"-method_{technique}" 
    print_text = print_text.lower()
    print(print_text)
    print(f'retain text = {retain_texts}, preserve_scale = {preserve_scale}, erase_scale = {erase_scale}')
