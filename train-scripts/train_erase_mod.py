...
def text2image_ldm_stable(
    ...
    def get_embeddings(prompt, uncond = False, max_length=model.tokenizer.model_max_length):
        if uncond:
            negative_prompt = negative_prompt_2 = batch_size * [""]
            prompts = [negative_prompt, negative_prompt_2]
        else:
        ...
        for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
            ...
            max_length = max(max_length, text_inputs.input_ids.shape[-1])
            prompt_embeds = text_encoder(text_input_ids.to(device))
            # prompt_embeds = prompt_embeds.hidden_states[-2]
            prompt_embeds_list.append(prompt_embeds)
        return prompt_embeds_list
        
    # prompt_embeds = torch.cat(get_embeddings(prompt), dim=-1)
    ...
    # # text_embeddings_1 = model.text_encoder(text_input.input_ids.to(model.device))[0]
    # # text_embeddings_2 = model.text_encoder_2(text_input.input_ids.to(model.device))[0]
    ...
    # uncond_embeddings_1 = model.text_encoder(uncond_input.input_ids.to(model.device))[0]
    # uncond_embeddings_2 = model.text_encoder_2(uncond_input.input_ids.to(model.device))[0]
    
    # Concatenate unconditional embeddings from both encoders
    # uncond_embeddings = torch.cat([uncond_embeddings_1, uncond_embeddings_2], dim=-1)
    # uncond_embeddings = model.text_encoder(uncond_input.input_ids.to(model.device))[0]
    # uncomd_embeddings = torch.cat(get_embeddings("", uncond=True, max_length=max_length), dim=-1)
...
def get_ratios(ldm_stable, prev_ratio, ratio_diff, max_ratio_gap, concepts, classes, num_samples=10, num_loops=3):
    ...
    #                 break
                ...
                images = ldm_stable(prompt,num_images_per_prompt=num_samples, num_inference_steps=4, generator = g).images
...
def edit_model(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale = 0.1, preserve_scale = 0.1, with_to_k=True, technique='tensor'):
    ...
    for layer_num in range(len(projection_matrices)):
        ...
        with torch.no_grad():
            ...
            tokenizers = [ldm_stable.tokenizer, ldm_stable.tokenizer_2] if ldm_stable.tokenizer is not None else [ldm_stable.tokenizer_2]
            text_encoders = (
                    [ldm_stable.text_encoder, ldm_stable.text_encoder_2] if ldm_stable.text_encoder is not None else [ldm_stable.text_encoder_2]
            )

            def get_embeddings(prompt, prompt_2 = None, uncond = False, max_length=ldm_stable.tokenizer.model_max_length):
                if uncond:
                    negative_prompt = negative_prompt_2 = batch_size * [""]
                    prompts = [negative_prompt, negative_prompt_2]
                else:
    ...
    # prompt_embeds = torch.cat(get_embeddings(prompt), dim=-1)
            ...
            for cnt, t in enumerate(zip(old_texts, new_texts)):
                ...
                # text_embeddings_1 = ldm_stable.text_encoder(text_input.input_ids.to(ldm_stable.device))[0]
                # text_embeddings_2 = ldm_stable.text_encoder_2(text_input.input_ids.to(ldm_stable.device))[0]
                # # text_embeddings = torch.cat([text_embeddings_1, text_embeddings_2], dim=-1)
                ...
                # old_emb = torch.cat(get_embeddings(old_text), dim=-1)
                old_emd = text_embeddings[0]
                old_emb = old_emb[final_token_idx:len(old_emb)-max(0,farthest-final_token_idx)]
                # new_emb = torch.cat(get_embeddings(new_text), dim=-1)
            ...
            for old_text, new_text in zip(ret_texts, ret_texts):
                ...
                text_embeddings = ldm_stable.text_encoder(text_input.input_ids.to(ldm_stable.device))[0]
                # text_embeddings = torch.cat(get_embeddings([old_text, new_text]), dim=-1)
...
if __name__ == '__main__':
    ...
    ldm_stable.text_encoder_2 = None
