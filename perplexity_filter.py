...
def train_detector(subset_size=10000, batch_size=32, device="cuda:6"):
...
def detect_adversarial(prompt, classifier, gpt2_model, tokenizer):
    """Detect if a prompt is adversarial"""
    features = prepare_features([prompt], gpt2_model, tokenizer, 'cuda')
...
classifier, gpt2_model, tokenizer = train_detector(subset_size=None, batch_size=32, device="cuda:6")
save_models(classifier, gpt2_model, tokenizer)

# classifer = lightgbm.Booster(model_file='lgbr_base.txt')
classifier_2 = joblib.load('models/lgb2.pkl')
...
result = detect_adversarial(test_prompt, classifier_2, gpt2_model, tokenizer)
print(f"\nTest Result:")
